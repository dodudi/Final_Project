...
<body>
	...
	<section class="contact_area section_gap">
		...
		<br> <br>
		<div class="container">
			<form action="${pageContext.request.contextPath}/option/memberCheck"
				method="post">
				<div class="row">
					<div class="col-md-12">
						<div class="contact_info">
							<div class="info_item">
								...
								<table class="table" id="checkTable">
									...
									<tbody>
										<c:forEach var="dl" items="${ori_dates}">
											<tr>
												<th><input type="text" name="dateList" value="${dl}"
													onfocus="this.blur();"></th>

												<th>${ori_dates.size() } <input type="text"
													name="ROOM_ID" value="${rez.ROOM_ID}"
													onfocus="this.blur();">
												</th>

												<td>
													<!-- 체크아웃 날짜는 숙박하지 않으므로 객실 금액이 부과되지 않는다 (0원) --> <c:if
														test="${dl != rez.REZ_CHECKOUT}">
														<span> <fmt:formatNumber value="${room.ROOM_PRICE}"
																pattern="#,###" />원
														</span>
													</c:if> <c:if test="${dl == rez.REZ_CHECKOUT}">
															...
															<c:if test="${d eq dl}">
																<c:choose>
																	<c:when test="${fn:contains(date.value, 'bf')}">
																		<th>[조식]</th>
																		<td><c:forEach var="people" items="${date.value}">
																				<!-- Map -->
																				<c:if test="${fn:contains(people.key, 'Adult')}">
																					<c:set var="bfAdult" value="${people.value}" />
				                            										성인:&nbsp;<input type="text"
																						name="bfAdult" value="${bfAdult}"
																						onfocus="this.blur();" style="width: 15px">명&nbsp;&nbsp;&nbsp;&nbsp;
						                            							</c:if>
																				<c:if test="${fn:contains(people.key, 'Child')}">
																					<c:set var="bfChild" value="${people.value}" />
						                            								아동:&nbsp;<input type="text"
																						name="bfChild" value="${bfChild}"
																						onfocus="this.blur();" style="width: 15px">명
						                            							</c:if>
																			</c:forEach></td>
																		<td><c:forEach var="oi" items="${optionInfo}">
																				<c:if test="${oi.OPTION_NAME eq '조식'}">
																					<span><fmt:formatNumber
																							value="${bfAdult * oi.OPTION_DEFAULT_PRICE + bfChild * oi.OPTION_CHILD_PRICE}"
																							pattern="#,###" />원</span>
																				</c:if>
																			</c:forEach></td>
																	</c:when>
																	<c:when test="${fn:contains(date.value, 'dn')}">
																		...
																		<td><c:forEach var="people" items="${date.value}">
																				...
																				<c:if test="${fn:contains(people.key, 'Adult')}">
						                            								...
						                            								성인:&nbsp;<input type="text"
																						name="dnAdult" value="${dnAdult}"
																						onfocus="this.blur();" style="width: 15px">명&nbsp;&nbsp;&nbsp;&nbsp;
						                            							</c:if>
																				<c:if test="${fn:contains(people.key, 'Child')}">
																					<c:set var="dnChild" value="${people.value}" />
						                            								아동:&nbsp;<input type="text"
																						name="dnChild" value="${dnChild}"
																						onfocus="this.blur();" style="width: 15px">명
						                            							</c:if>
																			</c:forEach></td>
																		<td><c:forEach var="oi" items="${optionInfo}">
																				<c:if test="${oi.OPTION_NAME eq '디너'}">
																					<span><fmt:formatNumber
																							value="${dnAdult * oi.OPTION_DEFAULT_PRICE + dnChild * oi.OPTION_CHILD_PRICE}"
																							pattern="#,###" />원</span>
																				</c:if>
																			</c:forEach></td>
																	</c:when>
																	<c:when test="${fn:contains(date.value, 'sp')}">
																		...
																		<td><c:forEach var="people" items="${date.value}">
																				...
																				<c:if test="${fn:contains(people.key, 'Adult')}">
						                            								...
						                            								성인:&nbsp;<input type="text"
																						name="spAdult" value="${spAdult}"
																						onfocus="this.blur();" style="width: 15px">명&nbsp;&nbsp;&nbsp;&nbsp;
						                            							</c:if>
																				<c:if test="${fn:contains(people.key, 'Child')}">
																					<c:set var="spChild" value="${people.value}" />
						                            								아동:&nbsp;<input type="text"
																						name="spChild" value="${spChild}"
																						onfocus="this.blur();" style="width: 15px">명
						                            							</c:if>
																			</c:forEach></td>
																		<td><c:forEach var="oi" items="${optionInfo}">
																				<c:if test="${oi.OPTION_NAME eq '수영장'}">
																					<span><fmt:formatNumber
																							value="${spAdult * oi.OPTION_DEFAULT_PRICE + spChild * oi.OPTION_CHILD_PRICE}"
																							pattern="#,###" />원</span>
																				</c:if>
																			</c:forEach></td>
																	</c:when>
									...
									<tfoot>
										<tr>
											<th colspan="3" style="text-align: right">총
									...
									<tbody>
										<c:forEach var="dl" items="${dateList}">
											<tr>
												<th><input type="text" name="dateList" value="${dl}"
													onfocus="this.blur();"></th>
												<th>객실명or아이디<input type="text" name="ROOM_ID"
													value="${rez.ROOM_ID}" onfocus="this.blur();"></th>
												<td>
													<!-- 체크아웃 날짜는 숙박하지 않으므로 객실 금액이 부과되지 않는다 (0원) --> <c:if
														test="${dl != rez.REZ_CHECKOUT}">
														<span><fmt:formatNumber value="${room.ROOM_PRICE}"
					...
					<div class="col-md-12 text-right">
						<button type="submit" value="submit"
							class="btn theme_btn button_hover">회원예약</button>
					</div>
				</div>
